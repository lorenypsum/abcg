...
void GameObjects::create() {
  ...
  createDistraction(assetsPath);
  createTarget(assetsPath);
}

void GameObjects::createDistraction(const std::string &assetsPath) {
...
void GameObjects::createTarget(const std::string &assetsPath) {
  ...
  // m_Kd = m_target.getKd();
  // m_Ks = m_target.getKs();
  // m_shininess = m_target.getShininess();
...
void GameObjects::randomizeStar(Star &star) {
...
void GameObjects::randomizeStarx(Starx &starx) {
  // Random position: x and y in [-20, 20), z in [-100, 0)
...
void GameObjects::paint() {
  ...
  renderDistraction(KaLoc, KdLoc, KsLoc, shininessLoc, modelMatrixLoc);
  renderTarget(KaLoc, KdLoc, KsLoc, shininessLoc, modelMatrixLoc);

  abcg::glUseProgram(0);
}

void GameObjects::renderDistraction(const GLint KaLoc, const GLint KdLoc,
...
void GameObjects::renderTarget(const GLint KaLoc, const GLint KdLoc,
                               const GLint KsLoc, const GLint shininessLoc,
                               const GLint modelMatrixLoc) {
  // Set uniform variables for the current model
  glm::mat4 modelMatrixProt{1.0f};
